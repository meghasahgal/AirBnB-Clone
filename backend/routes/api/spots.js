...
router.post("/", requireAuth, restoreUser,validateSpot,async (req, res, next) => {
	...
	res.status(201), res.json(payload);
	// res.json({
	//   newSpot
	// })
});
...
router.post(
	...
	async (req, res, next) => {
		...
		if (existingReview) {
			...
			res.status(201), res.json(newReview);
		}
	}
);
...
router.get("/:spotId/bookings", requireAuth, async (req, res) => {
	...
	//check if not owner of spot
	if (req.user.id !== spot.userId) {
		res.json({ Bookings: notOwner });
	}
...
router.post(
	...
	async (req, res) => {
		...
		//if all else doesn't hold true, create the booking
			...
			//return booking
			res.status(200), res.json(newBooking);
		}
